...
fn parse_bundle(js: &str) -> Result<Vec<Calendar>> {
    ...
    let render_boundary_re =
        Regex::new(r#"\b[A-Za-z_$][A-Za-z0-9_$]*=function\(\)\{var t=this,e=t\.\$createElement"#)?;
    ...
    for (year, tag) in &tabs {
        ...
        let second = second_slice
            .map(|s| extract_v(s, &v_re))
            .unwrap_or_default();
    ...
    if out.is_empty() {
        return Err(anyhow!(
            "解析 bundle 后没拿到任何学年内容，simso 可能已改版"
        ));
    }
    Ok(out)
}
