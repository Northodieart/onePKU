...
const IAAA_REDIRECT: &str = "https://bdkj.pku.edu.cn/login/oauth";
...
pub async fn complete_bdkj_login(store: &Store, iaaa_token: &str, username: &str) -> Result<()> {
    ...
    let user_resp = client
        ...
        .await
        .context("校验会话失败")?;
    let user_url = user_resp.url().clone();
    ...
    // Validate the protected destination; the current service uses SESSION alone.
    if user_url.host_str() != Some("bdkj.pku.edu.cn")
        || user_url.path() != "/personal/user"
        || (user_body.contains("iaaa.pku.edu.cn") && user_body.contains("oauth.jsp"))
    {
        return Err(anyhow!("北大空间登录未通过验证"));
    }
