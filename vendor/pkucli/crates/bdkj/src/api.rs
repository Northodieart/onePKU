...
impl BdkjApi {
    ...
    pub fn from_session() -> Result<Self> {
        crate::login::load_session()?;
        let store = pkuinfo_common::session::Store::new("bdkj")?;
        Ok(Self::new(crate::client::build(store.load_cookie_store()?)?))
    }

    /// Read the official application gate before querying per-room time slots.
    pub async fn application_open(&self) -> Result<bool> {
        let response = self
            .http
        ...
        if response.url().path().contains("login") {
            return Err(anyhow!("北大空间登录已失效"));
        }
        let body = response.error_for_status()?.text().await?;
        Ok(!body.contains("未开放预约申请")
            && !body.contains("%E6%9C%AA%E5%BC%80%E6%94%BE%E9%A2%84%E7%BA%A6%E7%94%B3%E8%AF%B7"))
    }
    ...
    pub async fn history_time(&self, room_id: &str, start_date: &str) -> Result<Vec<HistoryTime>> {
        ...
        if body.contains("请先完善个人信息") {
            return Err(anyhow!("BDKJ_PROFILE_REQUIRED"));
        }
        let list: Vec<HistoryTime> =
            serde_json::from_str(&body).context("学校未返回有效的教室占用时段")?;
        Ok(list)
    }
    ...
    pub async fn list_applications(&self) -> Result<Vec<Application>> {
        ...
        if !body.contains("id=\"results\"") && !body.contains("id='results'") {
            return Err(anyhow!("预约记录页面未识别，请在原站核对登录"));
        }
