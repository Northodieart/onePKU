...
impl TreeholeApi {
    ...
    pub async fn from_session_noninteractive() -> Result<Self> {
        let store = Store::new(APP_NAME)?;
        let session = store
            .load_session()?
            .ok_or_else(|| anyhow!("未登录，请先运行 `treehole login`"))?;
        ...
        // 检查并完成短信验证（交互式）
        // GUI never reads stdin or sends an SMS implicitly.

        // 验证后保存更新的 cookies
        // Loading an existing GUI session is read-only.
    ...
    async fn course_token_exchange(&self) -> Result<()> {
        ...
        if self.noninteractive {
            Err(anyhow!("COURSE_SMS_REQUIRED"))
        } else {
    ...
    pub async fn get_coursetable(&self) -> Result<Vec<CourseRow>> {
        ...
        if data.get("flag").is_some_and(|v| v == "false" || v == false) {
            return Err(anyhow!("TIMETABLE_SOURCE_UNAVAILABLE"));
        }
        let courses = data
            .get("course")
            .and_then(|v| v.as_array())
...
fn parse_search_holes(data: &serde_json::Value) -> Result<Vec<Hole>> {
    let list = data
        .get("list")
        .and_then(|v| v.as_array())
        .ok_or_else(|| anyhow!("响应缺少 list 数组"))?;
    list.iter()
        .map(|v| {
            if val_i64(v, "pid") <= 0 || !v["text"].is_string() {
                return Err(anyhow!("帖子字段不完整"));
            }
            Ok(parse_hole(v))
        })
        .collect()
...
#[cfg(test)]
mod desktop_search_tests {
    use super::*;
    #[test]
    fn optional_tag_shape_does_not_drop_search_results() {
        let data = serde_json::json!({"list":[{"pid":1,"text":"synthetic","timestamp":1,"tags_info":{}},{"pid":2,"text":"synthetic","timestamp":2,"tags_info":[]}]});
        assert_eq!(parse_search_holes(&data).unwrap().len(), 2);
        assert!(parse_search_holes(&serde_json::json!({"list":{}})).is_err());
    }
}
