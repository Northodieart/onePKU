...
- 完成度呈现：毕业总学分、各大类、每个学分系列各一页，左右滑动切换；圆环和直线进度条都按桌面端配色画出"在修"淡色段。大类总额按子系列求和、毕业总学分按大类求和，结果不越出方案自述的区间
...
- 课程卡整张可点（已识别的、待确认的、不计入的都一样），弹同一个编辑框改学分与归类，改完停留在原页；匹配不上的课程进"待确认"。教学网课程列表没有学分字段，在修课程要点课程卡手动填，填了才算进淡色弧与缺口
