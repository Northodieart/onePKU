...
- 培养方案：离线内置教务部公开的 2021/2023/2024/2025 版共 599 份方案（构建时从仓库根 `data/curriculum/` 同步，约 2MB）。按成绩与在修课程推断年级与专业（门户「单位」命中时先限定本院系，零重合不猜）；毕业总学分与各学分系列以圆环呈现，每类一页、左右滑动切换，每页分"已修课程 N 门"和"在修课程 N 门"两段列出该类及子系列的课程，条目右侧直接写拿到多少学分。有些方案不把大类总额写进要求表，这类回退到课程组的学分区间；只在课程组里出现、要求表漏掉的子系列（如物理学院的专业核心课）会补回并给出推算的学分下限。像专业核心课这种父类不给总额、下面按方向分列的，选择方案页可以多选一个细分方向：选定后这一类按该方向的学分计、大类总额按各子系列求和重算，只有别的方向独有的课不再算进这一类。匹配不上的课程进入"待确认"。课程卡整张可点（已识别的、待确认的、不计入的都一样），弹同一个编辑框改学分和归类，改完停留在原页。教学网的课程列表里没有学分字段，在修课程要点课程卡手动填，填了才算进圆环的淡色弧与缺口。大学英语按 Y/A/B/C/C+/免修 分级计 8/8/6/4/2/0 学分，不足 8 学分的差额按方案要求计入通识教育课。方案原文提供教务部整卷入口，不在应用内抽页
