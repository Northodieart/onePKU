...
- 培养方案：离线内置教务部公开的 2021/2023/2024/2025 版共 599 份方案（构建时从仓库根 `data/curriculum/` 同步，约 2MB）。按成绩与在修课程推断年级与专业（门户「单位」命中时先限定本院系，零重合不猜）；毕业总学分与各学分系列以圆环呈现，每类一页、左右滑动切换，每页都列出该类及子系列已计入的课程；有些方案不把大类总额写进要求表，这类回退到课程组的学分区间，只在课程组里出现、要求表漏掉的子系列（如物理学院的专业核心课）会补回并给出推算的学分下限；匹配不上的课程进入"待确认"，可手动归入某一类或标记不计入，归入后停留在原页；在修的课程不计入统计，只在页尾单列。大学英语按 Y/A/B/C/C+/免修 分级计 8/8/6/4/2/0 学分，不足 8 学分的差额按方案要求计入通识教育课。方案原文提供教务部整卷入口，不在应用内抽页
